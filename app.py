...
def migrate_db():
    ...
    try:
        cur.execute("""
            ...
            WHERE table_name='material_logs' AND column_name='serial_nos'
        """)
        if not cur.fetchone():
            cur.execute("ALTER TABLE material_logs ADD COLUMN serial_nos TEXT")
            print("✅ Added serial_nos to material_logs")

        cur.execute("""
            SELECT column_name FROM information_schema.columns
            WHERE table_name='consumable_logs' AND column_name='item_name'
        """)
        if not cur.fetchone():
            cur.execute("ALTER TABLE consumable_logs ADD COLUMN item_name TEXT")
            print("✅ Added item_name to consumable_logs")

        cur.execute("""
            SELECT column_name FROM information_schema.columns
            WHERE table_name='consumable_stock' AND column_name='item_name'
        """)
        if not cur.fetchone():
            cur.execute("ALTER TABLE consumable_stock ADD COLUMN item_name TEXT")
            print("✅ Added item_name to consumable_stock")
...
def dashboard():
    ...
    return render_template(
        'dashboard.html',
        total_in_stock=total_in_stock, fresh_c=fresh_c, ret_c=ret_c,
        issued_c=issued_c, faulty_c=faulty_c, fib_val=fib_val,
                ...
                if form_type == 'material':
                    ...
                    if act == 'Add New':
                        for i in range(qty):
                            ...
                            try:
                                cur.execute("INSERT INTO material_stock (item_code,item_name,quantity,serial_no) VALUES (%s,%s,1,%s)", (code, name, s))
                            except:
                                pass
                    elif act == 'Issue':
                        if serial:
...
def export_stb_status(status):
    ...
    conn = get_db()
    try:
        df = pd.read_sql(f"""SELECT stb_no as "Serial No", status as "Status", stock_type as "Type", dealer as "Dealer", updated_at as "Last Update" FROM stb_stock WHERE status = '{status}'""", conn)
        df = fix_timezone(df)
        release_db(conn)
