                    ...
                    if df.empty:
                        ...
                        for idx, row in df.iterrows():
                            ...
                            try:
                                ...
                                dist = _clean(row.get('distributor', ''))
                                ...
                                for fmt in ('%Y-%m-%d', '%Y-%m-%d %H:%M:%S', '%d-%m-%Y', '%d/%m/%Y', '%m-%d-%Y', '%Y/%m/%d'):
                                    try:
                                        ...
                                        continue
                                if not parsed_date:
                                    raise ValueError("Invalid date: " + raw_date)
                                ...
                                if cr:
                                    ...
                                    try:
                                        cur.execute("INSERT INTO lco_master (lco_code, lco_name, distributor) VALUES (%s, %s, %s) ON CONFLICT (lco_code) DO NOTHING", (lco_code, lco_name, dist))
                                    except:
                                        pass
                                if dist:
                                    try:
                                        cur.execute("UPDATE lco_master SET distributor = %s WHERE lco_code = %s AND (distributor IS NULL OR distributor = '')", (dist, lco_code))
                                    except:
                                        pass
                                try:
                                    cur.execute("INSERT INTO daily_active_summary (report_date, lco_code, active_count, deactive_count) VALUES (%s,%s,%s,%s) ON CONFLICT (report_date, lco_code) DO UPDATE SET active_count=EXCLUDED.active_count, deactive_count=EXCLUDED.deactive_count", (parsed_date, lco_code, act, deact))
                                except:
...
def da_sub_dists():
    ...
    if area:
        ...
        p.append(area)
    if dist:
        conds.append("COALESCE(distributor,'') = %s")
        p.append(dist)
    if conds:
...
def da_chart_data():
    ...
    cur = conn.cursor()
    try:
        cur.execute("SELECT 1 FROM information_schema.columns WHERE table_name='lco_master' AND column_name='distributor'")
        has_dist = cur.fetchone() is not None
    except:
        has_dist = False
    ...
    try:
        if has_dist:
            q = """SELECT d.report_date,
                   COALESCE(SUM(CASE WHEN COALESCE(m.distributor,'') NOT IN ('AROHON','') THEN d.active_count ELSE 0 END),0),
                   COALESCE(SUM(CASE WHEN COALESCE(m.distributor,'') NOT IN ('AROHON','') THEN d.deactive_count ELSE 0 END),0),
                   ...
                   FROM daily_active_summary d LEFT JOIN lco_master m ON d.lco_code=m.lco_code WHERE 1=1"""
            p = []
            if area:
                q += " AND m.area=%s"; p.append(area)
            if sub_dist:
                q += " AND m.sub_distributor=%s"; p.append(sub_dist)
            if dist:
                q += " AND COALESCE(m.distributor,'')=%s"; p.append(dist)
        else:
            q = """SELECT d.report_date, SUM(d.active_count), SUM(d.deactive_count), 0, 0
                   FROM daily_active_summary d LEFT JOIN lco_master m ON d.lco_code=m.lco_code WHERE 1=1"""
            p = []
            if area:
                q += " AND m.area=%s"; p.append(area)
        ...
        release_db(conn)
        try:
            cur2 = conn if conn else get_db()
            if not cur2: return jsonify({'dates':[],'full_dates':[],'kccl_a':[],'kccl_d':[],'arohon_a':[],'arohon_d':[]})
            cur2.execute("SELECT d.report_date, SUM(d.active_count), SUM(d.deactive_count) FROM daily_active_summary d GROUP BY d.report_date ORDER BY d.report_date ASC")
            rows2 = cur2.fetchall()
            cur2.close()
            if conn: release_db(conn)
            return jsonify({'dates':[r[0].strftime('%d-%b') for r in rows2],'full_dates':[r[0].strftime('%Y-%m-%d') for r in rows2],'kccl_a':[r[1] for r in rows2],'kccl_d':[r[2] for r in rows2],'arohon_a':[0]*len(rows2),'arohon_d':[0]*len(rows2)})
        except:
    ...
    try:
        if has_dist:
            ...
            q = """SELECT COALESCE(t1.lco_code,t2.lco_code), COALESCE(lm.lco_name,COALESCE(t1.lco_code,t2.lco_code)),
               ...
               LEFT JOIN lco_master lm ON COALESCE(t1.lco_code,t2.lco_code)=lm.lco_code"""
        p = [d_from, d_to]
        if has_dist and (area or sub_dist or dist):
            cs = []
            if area:
                cs.append("lm.area=%s"); p.append(area)
            if sub_dist:
                cs.append("lm.sub_distributor=%s"); p.append(sub_dist)
            if dist:
                cs.append("COALESCE(lm.distributor,'')=%s"); p.append(dist)
            if cs:
                q += " WHERE " + " AND ".join(cs)
        elif area or sub_dist:
            cs = []
            if area:
                cs.append("lm.area=%s"); p.append(area)
            if sub_dist:
        ...
        ka = sum(r[5] for r in rows if (r[4] or '').upper() == 'AROHON')
        kd = sum(r[7] for r in rows if (r[4] or '').upper() == 'AROHON')
        aa = total_active - ka
        ad = total_deactive - kd
        growth, churn = [], []
        tg, tc = 0, 0
        for r in rows:
            if mode == 'active':
                change = r[5] - r[4]
                prev_v, now_v = r[4], r[5]
            else:
                change = r[6] - r[7]
                prev_v, now_v = r[6], r[7]
        ...
        churn.sort(key=lambda x: x['change'])
        cur.close()
        release_db(conn)
        return jsonify({'kccl_active': aa, 'kccl_deactive': ad, 'arohon_active': ka, 'arohon_deactive': kd, 'total_active': total_active, 'total_deactive': total_deactive, 'total_growth': tg, 'total_churn': abs(tc), 'net': tg + tc, 'growth': growth, 'churn': churn, 'd_from': d_from, 'd_to': d_to, 'mode': mode})
    except Exception as e:
        print('[compare err]', e)
        try: cur.close()
        except: pass
...
def da_summary_tables():
    ...
    try:
        ...
        p = [date_val]
        if area:
            base += " AND m.area=%s"; p.append(area)
        if sub_dist:
            base += " AND m.sub_distributor=%s"; p.append(sub_dist)
        if dist:
            base += " AND COALESCE(m.distributor,'')=%s"; p.append(dist)
...
def _export_compare_base(d_from, d_to, mode, area, sub_dist, dist, positive):
    ...
    try:
        cur = conn.cursor()
        q = """SELECT COALESCE(t1.lco_code,t2.lco_code),COALESCE(lm.lco_name,''),COALESCE(lm.area,''),COALESCE(lm.sub_distributor,''),COALESCE(lm.distributor,''),
        ...
        if area or sub_dist or dist:
            ...
            if sub_dist:
                ...
                p.append(sub_dist)
            if dist:
                cs.append("COALESCE(lm.distributor,'')=%s")
                p.append(dist)
            q += " WHERE " + " AND ".join(cs)
        cur.execute(q, tuple(p))
        rows = cur.fetchall()
        ...
        for r in rows:
            ...
            if (positive and ch > 0) or (not positive and ch < 0):
                data.append({'LCO Code': r[0], 'LCO Name': r[1], 'Area': r[2], 'Sub Distributor': r[3], 'Distributor': r[4], 'Previous': r[4] if mode == 'active' else r[6], 'Current': r[5] if mode == 'active' else r[7], col_name: ch})
        return data, None
    except Exception as e:
        release_db(conn)
        return None, str(e)
...
def export_growth_report():
    ...
    d_to = request.args.get('to', '')
    mode = request.args.get('mode', 'active')
    data, err = _export_compare_base(d_from, d_to, mode, request.args.get('area', ''), request.args.get('sub_dist', ''), request.args.get('distributor', ''), True)
    if err:
    ...
    if not data:
        ...
        return redirect(url_for('daily_active'))
    return dl_excel(pd.DataFrame(data), f"Growth_{mode}_{d_from}_vs_{d_to}.xlsx")
...
def export_churn_report():
    ...
    d_to = request.args.get('to', '')
    mode = request.args.get('mode', 'active')
    data, err = _export_compare_base(d_from, d_to, mode, request.args.get('area', ''), request.args.get('sub_dist', ''), request.args.get('distributor', ''), False)
    if err:
        flash(f"Error: {err}", "error")
        return redirect(url_for('daily_active'))
    if not data:
        ...
        return redirect(url_for('daily_active'))
    return dl_excel(pd.DataFrame(data), f"Churn_{mode}_{d_from}_vs_{d_to}.xlsx")
